...
import com.cjl.message.listMessage.RPopMessage;
...
public class RPopHandler implements CommonHandler<RPopMessage> {
    @Synchronized
    @Override
    public ResponseMessage process(RPopMessage msg) throws Exception {
        ...
        if(search.getData() instanceof LinkedBlockingQueue){
            ...
            if (data.size() > 0) {
                return new ResponseMessage(ResultCode.SUCCESS_CODE, data.pollLast());
            } else{
