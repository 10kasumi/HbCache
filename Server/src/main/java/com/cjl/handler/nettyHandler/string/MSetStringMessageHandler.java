...
public class MSetStringMessageHandler extends SimpleChannelInboundHandler<MSetStringMessage> {
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, MSetStringMessage msg) throws Exception {
            ...
            for (int i = 0; i < keys.length; ++i) {
                ...
                if (slot >= HbServer.ClusterManager.getSlaveManager().getStart()
                    ...
                    for (NodeInfo slaveNode : slaveNodes) {
                        ...
                        channel.writeAndFlush(new SetStringMessage(keys[i], values[i]));
                    }
                } else {
                    for (ServerNode masterNode : HbServer.ClusterManager.getMasterNodes()) {
                        if (masterNode.getStart() >= slot && slot < masterNode.getStart()) {
                            ctx.channel().writeAndFlush(new RedirectMessage(masterNode.getNodeInfo().getHost(),
                                    masterNode.getNodeInfo().getPort(), new SetStringMessage(keys[i], values[i])));
                        }
                    }
                }
            }
