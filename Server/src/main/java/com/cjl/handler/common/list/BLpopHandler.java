...
import com.cjl.message.listMessage.BLpopMessage;
...
public class BLpopHandler implements CommonHandler<BLpopMessage> {
    @Synchronized
    @Override
    public ResponseMessage process(BLpopMessage msg) throws Exception {
        AtomicReference<ResponseMessage> message = new AtomicReference<>();
        CompletableFuture bLpopFuture = CompletableFuture.runAsync(() -> {
            ...
            while (true) {
                ...
                if (search.getData() instanceof LinkedBlockingQueue) {
                    ...
                    if (data.size() > 0) {
                        message.set(new ResponseMessage(ResultCode.SUCCESS_CODE, data.pollFirst()));
                    }
                } else {
